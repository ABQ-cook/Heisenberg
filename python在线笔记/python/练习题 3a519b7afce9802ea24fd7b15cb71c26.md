...
| 等级 | 主题 | 题号 | 对应你计划里的位置 |
| --- | --- | --- | --- |
| L1 入门 | dataclass 基础、默认值 | 1–2 | Day 2 上午 |
| L2 进阶 | `field()` 各项参数 | 3–5 | Day 2 上午延伸 |
| L3 专项 | `@property` 只读/校验/换算 | 6–8 | Day 2 下午（含计划里的"温度类"） |
| L4 综合 | dataclass + property 融合 | 9–10 | 直接升级你的 [Contact.py](http://contact.py/) |
| L5 挑战 | 小型实战（对标 Day 7 周考） | 11 | Week 1 收尾 |
...
2. 把 `done` 改成 `True` 后，新建一个 `Task("开会")`，确认它的 `done` 仍是 `False`（说明不可变默认值没问题）。
3. 思考：如果把某个字段默认值写成 `[]`（一个列表），会发生什么？→ 引出练习 3。
...
2. 确认 `created_at` 是自动生成的、构造时不用传。
3. 进阶：再加一个 `tags: set = field(default_factory=set, compare=False)`，让 `User` 的相等比较**只看 `username`**，忽略标签。
...
2. 写一个函数，遍历 `fields(Config)`，打印出每个字段名和它 `metadata` 里的 `"unit"`（没有就显示 "无单位"）。
...
2. 给 `ContactBook` 加 `@property def all_emails(self) -> set[str]`（只读，返回所有联系人的邮箱集合）。
3. 把 `Person` 的字段配置成：按 `name` 去重（`field(compare=True, hash=True)`），而 `phone`/`email` 设 `compare=False`（同名即视为同一人）。
4. 验证：`field(default_factory=dict, repr=False)` 的 `contacts` 在新建 `ContactBook()` 时确实是独立空字典。
...
2. **L3（约 2–3h）**：下午做练习 6–8，尤其是**练习 8 温度类**是你计划里点名的。
3. **L4（当晚或次日）**：练习 10 直接升级 `Contact.py`，练完等于把本周里程碑"通讯录 CLI"做到位。
4. **L5**：留到 Day 7 周考前当模拟题，不看笔记从零写。
